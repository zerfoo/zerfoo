...
func TestMockTrainingWorkflow(t *testing.T) {
	...
	if result == nil {
		t.Fatal("Expected training result, got nil")
	}
...
func TestPluginRegistryThreadSafety(t *testing.T) {
	...
	go func() {
		...
		for i := 0; i < 100; i++ {
			...
			_ = registry.RegisterWorkflow(fmt.Sprintf("workflow_%d", i), factory)
		}
	}()
...
func TestRegistrySummary(t *testing.T) {
	...
	_ = registry.RegisterWorkflow("test_workflow", workflowFactory)
	_ = registry.RegisterDataProvider("test_data", dataProviderFactory)

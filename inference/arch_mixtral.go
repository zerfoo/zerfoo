...
func init() {
	RegisterArchitecture("mixtral", buildMixtralGraph)
}

// buildMixtralGraph constructs a computation graph for the Mixtral architecture
// from pre-loaded GGUF tensors. It returns the graph and the embedding table
// tensor (needed by the generator for token lookup).
//
// Mixtral extends Mistral with Mixture of Experts in the FFN layer:
//   - All transformer layers use MoE (no dense FFN layers).
//   - Top-K expert routing (default top-2 out of 8 experts).
//   - GQA attention with sliding window (inherited from Mistral).
//   - RoPE positional encoding.
//   - RMSNorm layer normalization.
//   - SwiGLU activation within each expert FFN.
//   - No shared expert (unlike Llama 4 / DeepSeek).
//
// Expert tensor naming (stacked format, same as Llama 4 / DeepSeek):
//
...
// The architecture is:
//
//	Embed -> [RMSNorm -> GQA(sliding window, RoPE) -> Add -> RMSNorm -> MoE -> Add] x N -> RMSNorm -> LMHead
func buildMixtralGraph(
	...
	// Mixtral may tie lm_head to embedding weights.
	...
	if numExperts == 0 {
		numExperts = 8 // Mixtral default: 8 experts
	}
	topK := cfg.NumExpertsPerToken
	if topK == 0 {
		topK = 2 // Mixtral default: top-2 routing
	}
	...
	for i := 0; i < cfg.NumLayers; i++ {
		...
		// --- Self Attention (GQA with sliding window, inherited from Mistral) ---
		...
		// --- MoE (all Mixtral layers are MoE, no dense FFN fallback) ---
		ffnOut, err := buildMixtralMoE(
...
// buildMixtralMoE constructs the MoE sub-graph for a single Mixtral layer.
// Mixtral uses the same stacked expert format as Llama 4 and DeepSeek, but
// has no shared expert — only routed experts run per token.

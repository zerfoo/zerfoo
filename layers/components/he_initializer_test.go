...
func TestNewHeInitializer_FunctionalOptions(t *testing.T) {
	ops := numeric.Float32Ops{}

	// Test with functional options (even if no specific options are defined yet)
	initializer := NewHeInitializer(
		ops,
		// No specific options to pass yet, but demonstrating the pattern
	)

	testutils.AssertNotNil(t, initializer, "expected HeInitializer to not be nil")

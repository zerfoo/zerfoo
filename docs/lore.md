...
## L-0019: Batch generation checks out sessions concurrently but never runs them concurrently -- the shared graph mutex serializes every Forward, so "peak concurrent sessions" is not a testable property

**Tags:** #inference #concurrency #session-pool #flaky-test #gotcha
**Date:** 2026-08-20
**Repo:** zerfoo/zerfoo

**Rule:** Never assert that `inference.Model.GenerateBatch` (or any two sessions of one `generate.Generator`) run generations in parallel, and never measure concurrency by sampling `len(sessionPool)` from a polling goroutine. The testable invariants are session ISOLATION (no two live prompts hold the same `*generate.InferenceSession`) and pool ROUTING (the batch returns sessions through `releaseSession`); assert those with an arrival barrier and a pre-drained pool, never with a timing sample.

**Why:** `generate.InferenceSession.Generate` locks the Generator's shared `graphMu` for the ENTIRE call (`generate/session.go`, hoisted from per-step in `9ef47892` for decode throughput) because `*graph.Graph` is not concurrency-safe -- the same hazard `TestModel_SpeculativeGenerate_ConcurrentWithNormalGenerate` guards as CONC-H1. Measured directly with an in-flight counter inside the shared graph node during a 32-prompt batch at pool size 8 on a 20-core host: peak concurrent `Forward` = 1, every run. So `GenerateBatch`'s goroutines only overlap in the window between `acquireSession()` and blocking on `graphMu`. `54051087` ("use session pool in GenerateBatch instead of generator mutex", C-002) landed a week AFTER that hoist and shipped `TestGenerateBatch_ConcurrentSessions`, whose commit message premise -- "each goroutine gets its own session with an independent mutex, enabling true concurrent batch execution" -- was already false when written. The test polled `len(pool)` from a spinning goroutine and required `peak >= 2`; on `df1cb697` it failed 14 of 20 `-count=20` runs, reading `peak = 0` (the poller never sampled inside the checkout window at all) or `peak = 1`. Each failure took 0.00s, which is why it read as a scheduling race rather than a timeout. A `testing.Short()` skip had already been bolted on -- the ztensor#182 anti-pattern -- and did nothing, because CI does not pass `-short`. Session pooling is still worth having: it keeps per-prompt KV cache and position state isolated and preserves GPU pointers for CUDA graph replay. It just buys no wall-clock parallelism until the graph itself is concurrency-safe.
...
## L-0021: A per-layer reader must never take state from a cache accessor that reports layer 0 -- and prefill/decode equivalence is the only test that catches it

**Tags:** #kvcache #rope #decode #attention #vacuous-green #critical
**Date:** 2026-08-20
**Repo:** zerfoo/zerfoo

**Rule:** Anything an attention layer reads per layer -- above all the RoPE position offset -- must come from that layer's own cache state (`generate.LayerSeqLenProvider.LayerSeqLen(layer)`), never from `CacheProvider.SeqLen()`. `SeqLen()` answers from layer 0 in every implementation except `GPUKVCache`, and layer 0 advances its cursor inside its own `Forward`, so the value a caller sees depends on where in the stack it runs. Any new cache type must either implement `LayerSeqLen` or make `SeqLen` pass-stable the way `GPUKVCache` does (advance only after the last layer's append). Correspondingly, every KV-cache correctness test must assert **prefill(N) + decode == prefill(N+k)** on real values -- shapes, finiteness, and cursor counts do not discriminate.
**Why:** zerfoo#990: multi-token greedy decode diverged from llama.cpp from the second token onward, on byte-identical GGUFs, on `gemma3` and `qwen3` alike -- correct first token, then fluent on-topic repetition. `GroupedQueryAttention` set `rope.SetPositionOffset(cache.SeqLen())`, so during a 6-token prefill layer 0 saw 0 (correct) and layers 1-25 saw 6, rotating Q/K to positions [6,12) instead of [0,6). That shift is UNIFORM within one pass and RoPE is relative, so a pure prefill and a pure token-at-a-time decode are each internally consistent and look perfectly correct -- which is why first-token parity passed 6/8 and why the bug survived every existing test. It only bites at the prefill->decode boundary: after an N-token prefill, layers 1+ hold keys at positions [N,2N) while the first decode token is rotated to N+1, and every later query is scored against scrambled relative offsets. Measured on `gemma3-1b-it-Q4_K_M` (CPU, greedy): splitting the same 6-token prompt as prefill(k)+decode gave last-position logit maxAbsDiff of 7.45 / 6.53 / 4.64 / 0.84 / 0.00006 for k = 5/4/3/2/1 -- error proportional to the prefill chunk, and ZERO at k=1 because a one-token prefill degenerates to pure decode. `layers/attention/grouped_query_attention_cache_test.go`'s `TestGQA_CachedForward` had the right shape but explicitly declined to compare cached against uncached values ("outputs may differ slightly ... we verify the shapes are correct"), so it green-lit the defect for its whole lifetime (see L-0009). The replacement, `TestGQA_PrefillDecodeRoPEPositionParity`, asserts its own sensitivity before it asserts parity: an earlier draft used the default random weight init, which saturated the softmax onto one key and made the stack position-blind, so it passed with the bug present.
**Trigger:** Any `cache.SeqLen()` read from inside a per-layer `Forward`; a new `CacheProvider` implementation with no `LayerSeqLen`; a KV-cache test whose assertions are shapes, NaN checks, or cursor values rather than a prefill/decode value comparison; a cached-attention test that runs only one layer (a single-layer stack cannot see this class of bug at all).
**Source:** zerfoo#990; `generate/context.go` (`LayerSeqLenProvider`); `layers/attention/grouped_query_attention.go` (`cachedPositions`); `layers/attention/gqa_rope_position_parity_test.go`.

## L-0022: `CompileTraced` cannot produce a runnable plan for any inference-built graph -- tensors made in plain Go have no producing instruction, and 341 slots dangle, not one

**Tags:** #compile #execution-plan #tracer #megakernel #dead-code #gotcha
**Date:** 2026-08-23
**Repo:** zerfoo/zerfoo

**Rule:** Do not re-enable `graph.CompileTraced` in `generate.compileGraph` (it is opt-in behind `ZERFOO_TRACED_PLAN=1`) on the strength of fixing one node. The traced plan is built purely from ops the `EngineProxy` records, so EVERY tensor a node materialises in plain Go -- not just the embedding row lookup -- lands in a slot with no producer, and `RunInstructions` bails on the first one it reads. Before claiming the traced path works for an architecture, count the dangling slots: for each instruction input, a slot that is neither frozen, nor a plan input, nor an earlier instruction's output. The count must be zero.

**Why:** #994 identified `embeddingLookupNode.Forward` (`inference/arch_llama.go`) as the reason every generation logged `CompileTraced plan validation failed ... instruction 0 (Mul): input tensors cannot be nil`, and proposed teaching the tracer about that one node. Instrumenting the traced plan showed the embedding is merely the FIRST of many: gemma3-1b-it-Q4_K_M has **341 dangling slots of 1701** (288 frozen, 1 input), Qwen3-0.6B-Q8_0 has **283 of 1831** -- roughly 13 per transformer layer (RoPE tables, KV cache slices, attention masks, reciprocals). Fixing the embedding moves the failure from instruction 0 to instruction 1. Worse, a *working* traced plan would be slower on CPU, not faster: tracing decomposes the model into generic fp32 primitives (gemma3: 1071 instructions, 235 `MatMul`, 314 `Reshape`) and discards every fused/quantised kernel the node-level `Compile` plan uses (185 instructions: 26 `FFN`, 26 `GroupedQueryAttention`, 26 `FusedAddRMSNorm`, 26 `FusedNormAdd`, 53 `RMSNorm`). Its only genuine payoff is the megakernel emitter, which is CUDA codegen. All 25 `SetEngineProxy` call sites in the repo are inference architecture builders, so `proxy != nil` meant "GGUF graph" and therefore "tracing will fail" -- 100% of the time, on CPU and GPU alike. Cost of the dead attempt, measured on CPU: ~0.7s of wasted trace forward per generator (gemma3 697-765ms; Qwen3 321ms) plus a 35-50us failed validation, against 50-100us for the `Compile` fallback, which reuses `g.memo` and runs no forward at all.

**Trigger:** Any proposal to "just make the embedding emit a `Gather`"; any claim that the megakernel path is reachable for a GGUF model; any new node whose `Forward` builds its output with `tensor.New` instead of an engine call, if traced compilation is ever revived.

**Source:** issue #994; `generate/generator.go` (`compileGraph`, `tracedPlanEnabled`); `generate/compile_traced_test.go`; `ztensor@v1.19.2/graph/compile.go` (`CompileTraced`, `RunInstructions` L545); `ztensor@v1.19.2/compute/tracer.go`.

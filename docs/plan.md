...
- [x] T47.2.4 Batch attention forward and backward across all samples  Owner: TBD  Est: 3h  verifies: [UC-TS01]  DONE 2026-03-30
  ...
  The forward attention (lines ~610-663) and backward attention (lines ~1020-1061) still loop
  per-sample on CPU with nested scalar loops for Q@K^T, softmax, and V aggregation. All other
  ops (Q/K/V projections, FFN, output projection) are already batched as single GPU MatMul calls.
  Fix: (1) Reshape Q/K/V from [bs*numPatches, dModel] to [bs*nHeads, numPatches, headDim] via
  engine.Reshape + engine.Transpose. (2) Use engine.MatMul (3D batched) for Q @ K^T to get
  scores [bs*nHeads, numPatches, numPatches]. (3) Scale by 1/sqrt(headDim) via engine.MulScalar.
  (4) Use engine.Softmax(ctx, scores, -1) for batched softmax. (5) Use engine.MatMul for
  scores @ V to get [bs*nHeads, numPatches, headDim]. (6) Transpose and reshape back to
  [bs*numPatches, dModel]. Same approach for backward: reshape dAttnOut, use batched MatMul
  for dScores = dAttnOut @ V^T, dV = scores^T @ dAttnOut, softmax backward via
  dLogit = scores * (dScores - row_sum(scores * dScores)) * scale, then dQ = dLogit @ K,
  dK = dLogit^T @ Q. Replace lc.scores [][][][]float32 with *tensor.TensorNumeric[float32]
  shaped [bs*nHeads, numPatches, numPatches].
  Acceptance: (1) Batched attention output matches per-sample output within 1e-5.
  (2) Gradient check -- numerical vs analytical match within 1e-3. (3) go test -race passes.

- [ ] T47.2.3 Wire PatchTST TrainWindowed to batched path  Owner: TBD  Est: 2h  verifies: [UC-TS01]
  Deps: T47.1.1, T47.2.1, T47.2.2, T47.2.4
...
- [ ] T47.2.3 Wire PatchTST to batched path  Deps: T47.1.1, T47.2.1, T47.2.2, T47.2.4

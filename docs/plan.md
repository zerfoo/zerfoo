...
- **main is clean, all docs-only commits pushed directly** (this repo's established convention for plan/devlog/lore/review-doc updates -- see recent `git log`). `go build/vet/test -short ./...` verified green on 2026-08-08 outside `tests/parity`, `timeseries`, `generate`'s GPU cache test, `internal/cuda/kernels`. **CORRECTION (2026-08-10): those were NOT "no GPU in this dev sandbox" as stated here** -- this environment has a real GB10 GPU, and the exclusion was based on an unverified assumption, not a checked fact. Two of those packages' failures turned out to be genuine bugs (fixed in PR #980); see the 2026-08-10 devlog entry.
...
- **CORRECTION (retracts an earlier claim in this same session): "DGX SSH is broken" was wrong.** This session's sandbox environment IS the DGX host itself (`aitopatom-bfc8`, GB10 GPU, aarch64) -- `ssh ndungu@192.168.86.250` was a self-call to its own network IP, which failed for a mundane loopback-auth reason and says nothing about real remote access. Verified directly on-box: `nvidia-smi` shows the GB10 GPU; `/var/lib/zerfoo/models` already exists with three GGUF files staged (`gemma3-1b-it-Q4_K_M.gguf` -- 815MB, `gemma-4-E2B-it-Q4_K_M.gguf` -- 3.1GB, `gemma-4-E2B-it-Q8_0.gguf` -- 7.2GB; all dated 2026-04, predating this plan -- someone provisioned these ad hoc and it was never recorded). `df -h /` shows 2.1TB free. **T136.2 is therefore partially done, not "blocked: awaiting host provisioning."** Remaining gap: docs/bench/manifests/validate-arm64.yaml's models `hostPath` mount is still commented out/absent (the file's own comment says to re-add it once the path exists on the host -- it now does). Next session (or later this one, pending confirmation): (1) re-add the `hostPath` mount now that `/var/lib/zerfoo/models` exists, (2) decide whether the already-staged gemma-4-E2B GGUF is sufficient to unblock T134.1 (gemma4e fix) now, (3) confirm with David before pulling the rest of the ~10-model flagship list (docs/verified-models.md) given the multi-hundred-GB scale (MiniMax-M2 229B alone could be very large) -- this is real production host disk, not a sandbox to experiment freely on.

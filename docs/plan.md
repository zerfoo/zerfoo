...
- **Merge order:** #979 (grpc) -> #978 (F4, rebased on top) -> #977 (T145.1, rebased on top of both). All three rebase-merged cleanly (no file overlap). Post-merge `go build ./... && go vet ./... && go test -short ./...` clean on `main` outside the four known GPU-only-failure packages (tests/parity, timeseries, generate, internal/cuda/kernels -- no GPU in this environment, confirmed identical against pristine `main`).

...
- [x] T136.2 Provision GGUF models on the DGX host  Owner: David  Est: 2h  verifies: [UC-H2-004]  kind: human -- DONE 2026-08-10, executed by an agent session running directly on the DGX (not remote SSH, see docs/devlog.md 2026-08-10 entry for how "agents cannot do this" turned out to be false once on-box). 9 of 11 T136.1 candidates staged in /var/lib/zerfoo/models (~32GB): Gemma 3 1B/4B, Gemma 4 Edge (Q4_K_M+Q8_0), Llama 3.2 3B, Mistral 7B, Qwen 2 7B, Phi-4 (Q4_K), DeepSeek-R1-Distill-1.5B. hostPath mount re-added to validate-arm64.yaml (PR #980). Residual: Llama 4 Scout (~65GB) and MiniMax-M2 (~129GB) deliberately deferred -- multi-hour downloads at this network's throughput, not attempted. Chronos-2 has no GGUF distribution (structural, not a provisioning gap).
...
- [x] T136.2 model provisioning  kind: human  (ask David at wave start; everything in Tracks B/D waits on it) -- DONE 2026-08-10 (9/11 models staged; see devlog)
...
### 2026 08 10 (latest) -- Change Summary: T136.2 done (9/11 models staged); T134.1/T134.2 done (gemma4e demoted); 2 real GPU bugs fixed, 3 filed

- **T136.2 (GGUF model provisioning) DONE.** Executed directly on the DGX host (this session's sandbox IS the DGX, not a remote environment -- see hand-off notes). 9 of 11 flagship-matrix models staged in `/var/lib/zerfoo/models` (~32GB): Gemma 3 1B/4B, Gemma 4 Edge x2 quants, Llama 3.2 3B, Mistral 7B, Qwen 2 7B, Phi-4, DeepSeek-R1-Distill-1.5B. Llama 4 Scout (~65GB) and MiniMax-M2 (~129GB) deliberately deferred given this network's demonstrated download throughput; Chronos-2 has no GGUF distribution. `hostPath` mount re-added to `docs/bench/manifests/validate-arm64.yaml`.
- **T134.1/T134.2 (gemma4e disposition) DONE.** The H21 fix candidate (native Q4_K embedding storage) was implemented, verified end-to-end on real GPU hardware against the actual staged model, and refuted -- degenerate output persists. `gemma4`/`gemma4e`/`gemma4moe` demoted to experimental (load-time warning in `inference/load_gguf.go`). `zerfoo#766` closed as attempted; `zerfoo#757` re-scoped to parked with a full H1-H21 history pointer.
- **Two real GPU bugs found and fixed** (PR #980): a test-fixture shape bug in `TestGPUParity_Conv1D`, and a SIGSEGV crash in `TestGPUParity_GQA` from an upstream `ztensor` kernel bug affecting any GQA model (Llama/Mistral/Qwen/Gemma) -- mitigated in zerfoo, tracked upstream at `zerfoo/ztensor#180`. Both were only discoverable because two earlier-session claims ("DGX SSH broken," "no GPU in this sandbox") were wrong and got corrected mid-session -- see the 2026-08-10 devlog entry for the full, honest account including the corrections themselves.
- **Three more real GPU bugs found, triaged as pre-existing, filed rather than deep-dived**: `zerfoo#981` (KV-cache key offset bug), `zerfoo#982` (intermittent Gather CUDA error), `zerfoo#983` (PatchTST training-convergence failure).
- **Two lore entries added**: L-0016 (huggingface.co downloads from this DGX host need `curl -4 --http1.1 --fail -C -`, not defaults, or they silently truncate) and L-0017 (use the harness's background-task tracking, not `nohup`+`disown`, for cross-turn background work).
- **Landed as 2 PRs, both merged**: #980 (Conv1D + GQA fixes, environmental corrections), #985 (T134.2 gemma4e demotion).
- **T136.3/T136.4 (parity + benchmark runs against the staged models) are next**, now genuinely unblocked -- not attempted this session.

### 2026 08 09 -- Change Summary: T145.2 done; Wave Sec-5 complete; Objective 6 / D7 CLOSED
...
- **Correction to an error made earlier this same session:** this session's sandbox IS the DGX host itself (hostname `aitopatom-bfc8`, GB10 GPU present at `192.168.86.250`'s own interface) -- not a remote environment needing SSH in. Attempting `ssh ndungu@192.168.86.250` from the box was a self-loopback-over-network-IP call that failed for a mundane reason unrelated to real access; it was wrongly reported as "DGX access is broken, for David too" in this file and in a sitrep. That claim is retracted. `/var/lib/zerfoo/models` already exists on the host with three GGUF files staged (`gemma3-1b-it-Q4_K_M.gguf`, `gemma-4-E2B-it-Q4_K_M.gguf`, `gemma-4-E2B-it-Q8_0.gguf`, dated 2026-04, predating this plan) -- T136.2 is partially done already and nobody had recorded it. The bench manifest's models `hostPath` mount is still NOT re-added (docs/bench/manifests/validate-arm64.yaml). See the corrected hand-off note below for next steps.
...
- **Found and fixed a second, unrelated pre-existing blocker: GO-2026-6061 in google.golang.org/grpc v1.79.3**, reachable via the `distributed/` package's gRPC client retry path. A new advisory published to the govulncheck DB sometime in the same 26-day window; blocked `govulncheck-gate.sh` (CICD-6) for every PR regardless of diff, same class as the earlier GO-2026-4918/x/net fix. Bumped to v1.82.1. Landed as PR #979.
- **Merge order:** #979 (grpc) -> #978 (F4, rebased on top) -> #977 (T145.1, rebased on top of both). All three rebase-merged cleanly (no file overlap). Post-merge `go build ./... && go vet ./... && go test -short ./...` clean on `main` outside `tests/parity`, `timeseries`, `generate`, `internal/cuda/kernels`, which were failing at the time. **CORRECTION (2026-08-10): those failures were wrongly attributed to "no GPU in this environment."** This environment has a real GPU; the actual causes were two independent, genuine bugs (a test-fixture shape bug in `TestGPUParity_Conv1D`, and a real SIGSEGV in `TestGPUParity_GQA` from an upstream `ztensor` kernel bug) found and fixed in PR #980. #977/#978/#979's own changes were not implicated -- each was independently verified via targeted package tests plus GitHub's own CI, which passed on all three -- but the "confirmed identical against pristine main, no GPU hardware" framing here was wrong. See the 2026-08-10 devlog entry for the full account.
- **Wave Sec-5 now has only T145.2 remaining** (re-verify every High finding's repro test on `main`, update docs/deep-reviews/002-full-codebase.md's status header, file the deferred tech-debt issues #974/#975/#976). That closes Objective 6 / D7 and unblocks T138.1 (plan Phase 2), which also still needs the T136.2 human GGUF-provisioning dependency resolved or descoped -- see Standing notes.
- Full findings writeup: docs/devlog.md, 2026-08-08 entry.
...
### SESSION HANDOFF 2026-08-10 (live state for the next session)

**Wave Sec-5 is done (Objective 6 / D7 closed 2026-08-09). T136.2 (GGUF model provisioning) is now done too.** Phase 1's remaining open thread is T136.3/T136.4 (parity + benchmark runs against the now-staged models) -> T136.5 (surface the matrix) -> T138.1 (plan Phase 2). Pick up here:

- **This session's sandbox IS the DGX host** (`aitopatom-bfc8`, real GB10 GPU, confirmed via `nvidia-smi`) -- not a remote environment needing SSH. If a future session opens in this same environment, you already have direct filesystem + GPU access; do not assume you need to SSH anywhere or that GPU-touching tests are unavailable. Two wrong claims from earlier sessions ("DGX SSH is broken," "no GPU in this sandbox") were corrected this session -- see the 2026-08-10 devlog entry for the full story and what they invalidated (nothing merged was actually wrong, but two devlog/plan explanations were).
- **T136.2 is DONE.** 9 of 11 flagship-matrix candidates are staged in `/var/lib/zerfoo/models` (~32GB, 2TB free): Gemma 3 1B/4B, Gemma 4 Edge (Q4_K_M+Q8_0), Llama 3.2 3B, Mistral 7B, Qwen 2 7B, Phi-4 (Q4_K, the closest available to Q4_K_M), DeepSeek-R1-Distill-1.5B. The `hostPath` mount is re-added to `docs/bench/manifests/validate-arm64.yaml` (PR #980). **Deliberately not fetched:** Llama 4 Scout (~65GB) and MiniMax-M2 (~129GB) -- both would take multiple hours at this network's demonstrated throughput (see `docs/lore.md` L-0016 before attempting: huggingface.co downloads from this host need `curl -4 --http1.1 --fail -C -`, not plain `curl -sL` or `zerfoo pull`, or they silently truncate). Chronos-2 has no GGUF distribution at all (not a provisioning gap).
- **T134.1/T134.2 are DONE.** The gemma4e H21 fix candidate was implemented, GPU-verified on the actual staged model, and refuted (degenerate output persists). `gemma4`/`gemma4e`/`gemma4moe` now log a load-time experimental warning (`inference/load_gguf.go`). `zerfoo#766` closed as attempted; `zerfoo#757` re-scoped to `parked` with a full H1-H21 history pointer and revival trigger. PR #985.
- **Two real GPU bugs found and fixed this session** (PR #980), surfaced only because the "no GPU" exclusion was wrong and got corrected: `TestGPUParity_Conv1D` (test-fixture shape bug, trivial fix) and a SIGSEGV in `TestGPUParity_GQA` from an upstream `ztensor` kernel bug (`RepeatInterleaveF32` null-pointer CUDA launch, affects any GQA model -- Llama/Mistral/Qwen/Gemma -- mitigated by disabling the fused path; tracked upstream at `zerfoo/ztensor#180`).
- **Three more real GPU bugs found, triaged as pre-existing (not caused by this session's changes), and filed rather than fixed** (time-boxed per Fable-5 guidance mid-session): `zerfoo#981` (KV-cache multi-head key offset bug), `zerfoo#982` (intermittent `cudaMemcpy invalid argument` in Gather), `zerfoo#983` (PatchTST tiny-training GPU convergence failure). Worth picking up in a future session.
- **Next concrete step: T136.3 (parity runs) and T136.4 (benchmark re-runs)** against the 9 staged models, now genuinely unblocked. Then T136.5 (surface the matrix in docs/verified-models.md/README) and T138.1 (plan Phase 2) -- T138.1's other blockers (T133.4, T135.6) are already done.
- **Standing lessons (do not skip):** (1) after any dense same-file parallel wave, run full `go build ./...` on main after merges. (2) Don't trust a prior session's "CI green" claim without re-checking `gh pr checks` yourself. (3) **Any environmental claim (no GPU, access broken, "expected failure due to sandbox limits") requires a direct probe of the claimed cause** -- `nvidia-smi`, `hostname`, an actual reproduction -- not a pattern-match on a failure signature. This session made two wrong claims of exactly that shape before catching them.

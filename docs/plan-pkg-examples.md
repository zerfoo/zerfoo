...
- [x] T1.1 Add NewModel constructor to api.go  Owner: TBD  Est: 0.5h  verifies: [infrastructure]
...
- [x] T1.2 Add NewModelWithEmbeddings constructor  Owner: TBD  Est: 0.5h  verifies: [infrastructure]
  File: api.go
...
- [x] T2.1 ExampleModel_Chat  Owner: TBD  Est: 0.5h  verifies: [UC-002]
...
- [x] T2.2 ExampleModel_Generate  Owner: TBD  Est: 0.5h  verifies: [UC-004]
  File: example_test.go
  Generate with WithMaxTokens, WithTemperature. Print result.
  Acceptance: deterministic output, test passes.

- [x] T2.3 ExampleModel_ChatStream  Owner: TBD  Est: 0.5h  verifies: [UC-003]
  File: example_test.go
  Stream tokens, print each. Verify channel closes.
  Acceptance: deterministic output, test passes.

- [x] T2.4 ExampleEmbedding_CosineSimilarity  Owner: TBD  Est: 0.5h  verifies: [UC-005]
  File: example_test.go
  Create two Embedding structs with known vectors.
  Print similarity with %.4f.
  Acceptance: exact numeric output matches.

- [x] T2.5 ExampleModel_Embed  Owner: TBD  Est: 0.5h  verifies: [UC-005]
  File: example_test.go
...
- [x] T3.1 ExampleModel_Generate_toolCalling  Owner: TBD  Est: 0.5h  verifies: [UC-011]
  File: example_test.go
...
- [x] T3.2 ExampleModel_Generate_structuredOutput  Owner: TBD  Est: 0.5h  verifies: [UC-012]
  File: example_test.go
...
- [x] T4.1 ExampleDetectToolCall (serve package)  Owner: TBD  Est: 0.5h  verifies: [UC-011]
...
- [x] T4.2 ExampleGrammar (grammar package)  Owner: TBD  Est: 0.5h  verifies: [UC-012]
...
- [x] T5.1 Run all examples with go test  Owner: TBD  Est: 0.5h  verifies: [infrastructure]
  Run: `go test -run Example -v ./...`
  All 9 examples must pass.
  Acceptance: zero failures.

- [x] T5.2 Run go vet on example files  Owner: TBD  Est: 0.5h  verifies: [infrastructure]
  Run: `go vet ./...`
  Acceptance: clean.
...
- [x] T1.1 NewModel constructor
- [x] T1.2 NewModelWithEmbeddings constructor
- [x] T4.1 ExampleDetectToolCall (serve, no deps)
- [x] T4.2 ExampleGrammar (grammar, no deps)

#### Wave 2: All API Examples (7 agents)
- [x] T2.1 ExampleModel_Chat  Deps: T1.1
- [x] T2.2 ExampleModel_Generate  Deps: T1.1
- [x] T2.3 ExampleModel_ChatStream  Deps: T1.1
- [x] T2.4 ExampleEmbedding_CosineSimilarity  (no deps)
- [x] T2.5 ExampleModel_Embed  Deps: T1.2
- [x] T3.1 ExampleModel_Generate_toolCalling  Deps: T1.1
- [x] T3.2 ExampleModel_Generate_structuredOutput  Deps: T1.1

#### Wave 3: Verification (2 agents)
- [x] T5.1 Run all examples  Deps: all above
- [x] T5.2 Run go vet  Deps: all above

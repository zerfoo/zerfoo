...
Once built, the `*graph.Graph[T]` object provides not only `Forward` and `Backward` methods for execution, but also a rich introspection API. This allows for detailed analysis and manipulation of the graph structure. Key introspection methods include:
- `GetAllNodes() []Node[T]`: Returns a slice of all nodes in the graph.
- `GetDependencies() map[Node[T]][]Node[T]`: Returns a map of node dependencies.
- `GetTopologicalOrder() ([]Node[T], error)`: Returns the nodes in a topologically sorted order, ready for execution.
- `GetNodeMetadata(n Node[T]) map[string]interface{}`: Retrieves metadata for a specific node, such as its operation type, attributes, and output shape.
This API is crucial for tasks like model serialization, visualization, and advanced graph transformations, providing a clean separation between the graph's structure and its execution.
...
To support custom layers and ensure the framework is extensible, Zerfoo's ZMF serialization mechanism is built around `NodeSerializer` and `NodeDeserializer` interfaces.
- `NodeSerializer`: Allows custom serialization logic for a specific node type (`OpType`). Developers can register a custom serializer to control how their layer is converted to the ZMF format.
- `NodeDeserializer`: The corresponding interface to parse a ZMF node and reconstruct the custom layer object during model loading.
This pluggable system, managed by the `ZMFExporter` and `ZMFLoader`, means that Zerfoo can save and load any model, even those with user-defined layers, as long as the corresponding serializers/deserializers are registered. This is a key feature for enabling research and experimentation with novel architectures.
...
*   **Core layers:** Linear, Dense, Bias, FFN, Dropout, Add, FiLM (Feature-wise Linear Modulation)
*   **Activations:** ReLU, Leaky ReLU, GELU, Fast GELU, Sigmoid, Tanh, Softmax, Swish/SiLU
*   **Attention mechanisms:** Single and multi-head attention, global attention, causal masking, Grouped Query Attention (GQA)
*   **Embeddings:** Token embedding and Rotary Positional Embedding (RoPE)
*   **Feature Extraction:** Spectral Fingerprint (FFT-based)

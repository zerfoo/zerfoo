...
## 2026-08-10: T136.2 GGUF provisioning session -- two wrong environmental claims corrected, a real GPU crash bug found and fixed, gemma4e H21 fix candidate refuted

**Type:** investigation + bugfix + correction
**Tags:** T136.2, gqa, cuda, ztensor, gemma4e, h21, sandbox-environment, conv1d, tests-parity

**Context.** Picked up T136.2 (GGUF model provisioning on the DGX for the verified-model matrix) and T134.1 (attempt the gemma4e H21 fix candidate) after the 2026-08-09 security closeout. What follows is the full, honest account, including two significant wrong claims made and then corrected mid-session -- recorded per standing guidance to distinguish "I observed X" from "I concluded X."

**Correction 1: "DGX SSH is broken, for David too" was wrong.** Ran `ssh ndungu@192.168.86.250` from this session, got `Permission denied`, and (before checking) told David that meant real remote access was broken. It didn't -- this session's sandbox IS the DGX host itself (`hostname` = `aitopatom-bfc8`, matching `nvidia-smi`'s GB10 and the box's own `192.168.86.250` interface). The failed SSH was a self-call to the box's own network IP, which fails for a mundane loopback-auth reason and proves nothing about remote access. Verified directly on-box instead: `/var/lib/zerfoo/models` already had three GGUF files staged from April, predating this plan and never recorded.

**Correction 2 (bigger): "no GPU in this sandbox" was also wrong.** Earlier in this same session (before either correction), a full `go test -short ./...` run showed `tests/parity`, `timeseries`, `generate`, and `internal/cuda/kernels` crashing/failing, and that was written up as "expected, no GPU hardware here, confirmed identical against pristine main" -- used to justify excluding those packages when verifying PRs #977 (T145.1 CLI flags), #978 (F4 GGUF fix), and #979 (grpc CVE bump). That premise was never actually checked against `nvidia-smi`; it was inferred from a crash signature. There IS a real GPU. Re-running those packages properly (see below) found the crashes were caused by two genuine, independent, previously-hidden bugs -- not GPU absence. The 3 merged PRs' own changes are not implicated (each was independently verified via targeted package tests plus GitHub's own CI, which passed on all three), but the local "confirmed identical against pristine main" framing in the prior devlog/plan entries was wrong and is corrected here.

**Standing rule adopted for the rest of this session and going forward: an environmental claim (no GPU, access broken, "expected failure") requires a direct probe of the claimed cause, not a pattern-match on a failure signature.** Both wrong conclusions above shared that same shape.

**Bug found + fixed: `TestGPUParity_Conv1D` was a test-fixture bug, not a product bug.** `layers/core/conv1d.go`'s `Conv1D.Forward` is documented and implemented as `[batch, inChannels, length]`; `tests/parity/gpu_parity_ops_test.go`'s `TestGPUParity_Conv1D` built its input tensor as `[batch, seqLen, inCh]` (length and channels swapped), so the very first channel-count validation inside `Forward` correctly rejected it ("input channels 8 != expected 3"). `layers/core/conv1d_test.go`'s own unit tests use the correct convention, confirming this was isolated to the parity harness. Fixed by swapping the tensor shape construction in both the CPU and GPU branches of the test. Now passes with `maxDiff=0.0` (exact CPU/GPU match).

**Bug found + mitigated: ztensor's fused `RepeatInterleaveF32` GPU kernel segfaults -- affects any GQA model.** `TestGPUParity_GQA` crashed with `SIGSEGV: segmentation violation, PC=0x0` inside a CUDA kernel launch, traced through `layers/attention/grouped_query_attention.go:965` -> `ztensor@v1.19.2`'s `compute.GPUEngine.RepeatInterleave` -> `gpuapi.CUDAKernels.RepeatInterleaveF32` -> the purego/cgo kernel call itself. This is the "fused RepeatInterleave" decode optimization added 2026-03-30 (`641a119b`, perf(attention): wire fused softmax+V and repeat-interleave for decode`), reached whenever `numQueryHeads != numKeyValueHeads` (true for essentially every modern GQA architecture -- Llama, Mistral, Qwen, Gemma) and SDPA runs without a flash-decode shortcut. Because it's a hard SIGSEGV (not a Go error return), the type-assertion gate around the fused path cannot protect against it and the existing Reshape->Repeat->Reshape fallback is never reached. Root cause is in `ztensor` (a sibling repo, not editable from this session) -- filed as `zerfoo/ztensor#180` with the full repro and stack trace. Mitigated in zerfoo by disabling the fused path unconditionally (`fusedRepeatInterleaveEnabled = false` in `grouped_query_attention.go`), forcing the safe fallback. Verified: crash gone, `TestGPUParity_GQA` passes with `maxDiff=7.6e-6`, full `tests/parity` suite now runs clean (`ok`, no failures) for the first time this session.

**T134.1 (H21 gemma4e fix candidate): REFUTED, degenerate output persists.** Ran the staged `gemma-4-E2B-it-Q4_K_M.gguf` through a minimal greedy-decode harness (top-k=1) twice, independently, with `ZERFOO_GEMMA4_PLE_NATIVE_Q4K=1` set, GPU otherwise idle both times (the first attempt overlapped with a `dgx-validate.sh` parity pod and was discarded as untrustworthy per the CUDA-context-poisoning hazard class in L-0013 -- re-run cleanly after the pod finished). Both clean runs produced identical deterministic garbage (`"n;n;n\n\nx\n\nx\n\n..."`), different from the baseline's garbage (`"olyolyoly..."`) but equally incoherent -- confirming the flag does change behavior (it's wired correctly) without fixing the underlying decode-quality bug. Per T134.1's own acceptance criteria, this routes to T134.2 (demote to experimental).

**Three more real, independent failures surfaced** while re-verifying the previously wrongly-excluded packages, not yet triaged for whether they're pre-existing or caused by the GQA mitigation above:
- `TestGPUTinyTrainingConvergence` (timeseries): loss increased across a tiny GPU training run (0.908 -> 1.222) instead of decreasing.
- `TestTensorCache_FP16_GPU_MultiHead_PrefillAndDecode` (generate): retrieved KV-cache keys are systematically offset from expected partway through a multi-head prefill+decode sequence (e.g. `Key[16]=17, want ~13`).
- `TestGatherInt64Parity/multiple_indices` (internal/cuda/kernels): `cudaMemcpy failed: invalid argument`, isolated to the multiple-indices sub-case.

**Triage of the three failures above: all confirmed pre-existing, none caused by the GQA mitigation.** Reverted `grouped_query_attention.go`'s fix locally (`git apply -R`) and re-ran all three -- each failed identically (same symptom, same or near-same values) with the fused RepeatInterleave path active, confirming they predate this session's change. Restored the fix (re-verified `TestGPUParity_GQA` still passes, `maxDiff=7.6e-6`) and filed each as its own tracked GitHub issue with full repro, since Conv1D/GQA already consumed real investigation time and three more independent GPU bugs is too much to deep-dive in one already-long session:
- `zerfoo/ztensor#180` -- the RepeatInterleaveF32 upstream root cause.
- `zerfoo#981` -- TensorCache FP16 multi-head KV-cache key offset bug.
- `zerfoo#982` -- TestGatherInt64Parity intermittent `cudaMemcpy invalid argument` (non-deterministic which sub-case fails).
- `zerfoo#983` -- PatchTST tiny-training GPU convergence failure (loss increases instead of decreasing).

**Landed as two PRs, both merged:** #980 (Conv1D fix + GQA crash mitigation + the two environmental corrections above) and #985 (T134.2 gemma4e disposition -- see below). #985's first CI run hit the same "fuzzing process hung or terminated unexpectedly: exit status 2" signature the F4 investigation saw on 2026-08-08, on a PR that doesn't touch `model/gguf/parser.go` at all; a rerun passed clean, consistent with that being CI-runner flakiness rather than a new bug (same misattribution risk noted in the F4 writeup).

**T134.2 executed: gemma4/gemma4e/gemma4moe demoted to experimental.** `inference/load_gguf.go` now logs `slog.Warn("loading experimental architecture", ...)` at load time for all three, verified firing against the staged gemma-4-E2B GGUF. `docs/verified-models.md` and `README.md`/`docs/design.md` already carried zero Gemma 4 mentions -- that part of the standing rule was already satisfied. Issue disposition: `zerfoo#766` (the H21 fix attempt) closed as attempted; `zerfoo#757` (the root-cause epic) re-scoped to `parked` with a full H1-H21 investigation-history pointer and an explicit revival trigger (a new, evidence-backed hypothesis -- not another open-ended sweep).

**Model provisioning (T136.2): 9 of the flagship list staged, 2 deliberately deferred.** `zerfoo pull`'s downloads to huggingface.co were unreliable from this host: repeated silent truncations (a download can exit 0 with a fraction of the expected bytes) and connection resets, root-caused to two stacked issues -- a broken IPv6 path to huggingface.co from this host, and HTTP/2 stream resets that `curl`/`zerfoo pull` don't treat as fatal without `--fail`. Worked around per-download with `curl -4 --http1.1 -sL --fail --retry 10 --retry-delay 3 --retry-all-errors -C -`, verifying every file's final size against the server's `Content-Length` before accepting it (deliberately not a system-wide `/etc/gai.conf` change, since this DGX runs other teams' pods -- shared-infrastructure changes need more than a solo call). Full lesson recorded as `docs/lore.md` L-0016. Separately, manually-`nohup`'d background downloads got silently reaped between agent turns (not OOM -- confirmed via `dmesg`/`journalctl`); switching to the harness's own background-task tracking fixed it (L-0017).

`gemma-3-4b-it-Q4_K_M.gguf`'s earlier checksum failures (via `zerfoo pull`, before the HTTP/2 root cause above was found) turned out to be a genuine HuggingFace-side metadata inconsistency, not a truncation artifact: an independent, size-verified `curl` download reproduced the exact same SHA-256 HuggingFace's own `X-Linked-Etag` header disagreed with -- the file's content is what it is, the header was simply stale.

**Final tally, `/var/lib/zerfoo/models/` (~32GB, 2TB free on `/`):** Gemma 3 1B, Gemma 3 4B, Gemma 4 Edge (Q4_K_M + Q8_0, both pre-existing), Llama 3.2 3B, Mistral 7B, Qwen 2 7B, Phi-4 (Q4_K -- no Q4_K_M variant exists in `microsoft/phi-4-gguf`, closest available), DeepSeek-R1-Distill-Qwen-1.5B. Re-added the models `hostPath` mount to `docs/bench/manifests/validate-arm64.yaml` (PR #980) now that the directory exists on the host.

**Deliberately deferred, not attempted:** Llama 4 Scout (~65GB, 2 shards) and MiniMax-M2 (~129GB, 3 shards) -- both would take multiple additional hours at this network's demonstrated throughput, disproportionate to the time already spent this session. Chronos-2 remains structurally unavailable as GGUF (per `docs/verified-models.md`, not a provisioning gap). T136.3 (parity runs) and T136.4 (benchmark re-runs) are separate follow-up tasks against the now-staged models; not attempted this session.
